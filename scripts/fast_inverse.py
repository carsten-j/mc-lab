...
if __name__ == "__main__":
    ...
    plot_configs = {
        'Gaussian': {
            ...
            'pdf': lambda x: (1 / (1.5 * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((x - 2) / 1.5)**2)
        },
        'Multimodal': {
            'domain': np.linspace(-8, 8, 1000),
            'pdf': lambda x: np.exp(-x**2 / 2) * (1 + np.sin(3*x)**2) * (1 + np.cos(5*x)**2)
        },
        'GUE Spectral': {
            'domain': np.linspace(-4, 4, 1000),
            'pdf': lambda x: np.exp(-4*x**2) * (9 + 72*x**2 - 192*x**4 + 512*x**6)
        },
        'Oscillatory': {
    ...
    for i, (name, sample_data) in enumerate(samples.items()):
        ...
        ax.hist(sample_data, bins=50, density=True, alpha=0.7, label='Samples', color='skyblue')
