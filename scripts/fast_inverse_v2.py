...
if __name__ == "__main__":
    ...
    y_gauss = (1 / (1.5 * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((x_gauss - 2) / 1.5)**2)
    ...
    print(f"Gaussian samples: mean={np.mean(gaussian_samples):.3f}, std={np.std(gaussian_samples):.3f}")
    print(f"Mixture samples: mean={np.mean(mixture_samples):.3f}, std={np.std(mixture_samples):.3f}")