...
class TestMetropolisHastingsSampler:
    ...
    def test_1d_standard_normal_sampling(self, random_seed):
        ...
        assert samples.shape == (2, n_samples), f"Expected (2, {n_samples}), got {samples.shape}"
        ...
        print(f"1D Normal - Mean: {sample_mean:.3f}, Std: {sample_std:.3f}, "
    ...
    def test_2d_multivariate_normal_sampling(self, random_seed):
        ...
        print(f"2D MVN - Mean error: {mean_error}, Corr error: {abs(sample_corr - true_corr):.3f}")
    ...
    def test_challenging_bimodal_distribution(self, random_seed):
        ...
        def log_bimodal(x):
            ...
            if log_comp1 > log_comp2:
                return log_w1 + log_comp1 + np.log(1 + np.exp(log_w2 - log_w1 + log_comp2 - log_comp1))
            else:
                return log_w2 + log_comp2 + np.log(1 + np.exp(log_w1 - log_w2 + log_comp1 - log_comp2))
        ...
        idata = sampler.sample(
            ...
            thin=1,
            initial_states=np.array([-3, -1, 2, 4]).reshape(-1, 1),  # Overdispersed starts
            random_seed=random_seed,
            progressbar=False,
        )
        ...
        assert abs(prop_left - 0.6) < 0.15, f"Left mode proportion {prop_left:.3f} vs expected 0.6"
        assert abs(prop_right - 0.4) < 0.15, f"Right mode proportion {prop_right:.3f} vs expected 0.4"
        
        # Check mode locations
        if len(left_mode_samples) > 100:
            left_mean = np.mean(left_mode_samples)
            assert abs(left_mean - (-2.0)) < 0.4, f"Left mode mean {left_mean:.3f} vs expected -2.0"
            
        if len(right_mode_samples) > 100:
            right_mean = np.mean(right_mode_samples)
            assert abs(right_mean - 3.0) < 0.4, f"Right mode mean {right_mean:.3f} vs expected 3.0"
    ...
    def test_adaptive_scaling_behavior(self, random_seed):
        ...
        assert 0.25 < accept_rate_small < 0.55, f"Small scale acceptance rate: {accept_rate_small:.3f}"
        assert 0.25 < accept_rate_large < 0.55, f"Large scale acceptance rate: {accept_rate_large:.3f}"
        
        print(f"Adaptive - Small: {initial_scale_small:.4f} -> {final_scale_small:.4f} "
              f"(accept: {accept_rate_small:.3f})")
        print(f"Adaptive - Large: {initial_scale_large:.4f} -> {final_scale_large:.4f} "
    ...
    def test_convergence_diagnostics_with_arviz(self, random_seed):
        ...
        for var in ["theta1", "theta2"]:
            ...
            assert rhat[var].values < 1.1, f"R-hat for {var}: {rhat[var].values:.4f} > 1.1"
            
        # Test effective sample size
        ess = az.ess(idata)
        for var in ["theta1", "theta2"]:
            assert var in ess, f"ESS should include {var}"
            assert ess[var].values > 100, f"ESS for {var}: {ess[var].values:.0f} too low"
            
        # Test MCSE (Monte Carlo Standard Error)
        mcse = az.mcse(idata)
        for var in ["theta1", "theta2"]:
            assert var in mcse, f"MCSE should include {var}"
            assert mcse[var].values < 0.1, f"MCSE for {var}: {mcse[var].values:.4f} too high"
            
        print(f"Convergence - R-hat: theta1={rhat['theta1'].values:.4f}, "
              f"theta2={rhat['theta2'].values:.4f}")
        print(f"Convergence - ESS: theta1={ess['theta1'].values:.0f}, "
    ...
    def test_performance_and_scaling(self, random_seed):
        ...
        samples = np.column_stack([
            idata.posterior[f"x{i}"].values.flatten() 
            for i in range(5)
        ])
        ...
        assert np.all(np.abs(sample_means) < 0.1), f"Means too far from 0: {sample_means}"
        assert np.all(np.abs(sample_stds - 1.0) < 0.15), f"Stds too far from 1: {sample_stds}"
    ...
    def test_tune_proposal_scale_method(self, random_seed):
        ...
        print(f"Tuning - Tuned scale: {tuned_scale:.3f}, "
