...
def pareto_smooth_is(weights, tail_fraction=0.2):
    ...
    tail_count = int(max(3, np.floor(tail_fraction * N)))  # at least 3 points in tail for fit
    if tail_count >= N:
    ...
    smoothed_weights[:threshold_index] = sorted_weights[:threshold_index]   # weights below threshold unchanged
...
outliers = np.random.uniform(low=50, high=100, size=5)        # 5 large weights between 50 and 100
...
print(f"Number of weights = {len(weights)}, Effective tail count = {len(weights) - np.searchsorted(np.sort(weights), smoothed_w[len(weights)-len(outliers)])}")