...
class QuasiRandomInverseTransformSampler(BaseInverseTransformSampler):
    """
    Quasi-random (low-discrepancy) inverse transform sampler.
    Works with both analytical and numerical inverse CDFs.
    """
    ...
    def _generate_uniform(self, n: int) -> np.ndarray:
        """Generate quasi-random uniform samples."""
        if self.sequence_type == "sobol":
            return self._sobol_sequence(n)
        else:
            return self._van_der_corput_sequence(n, base=2)

    def _sobol_sequence(self, n: int) -> np.ndarray:
        ...
        try:
            ...
            if self.random_state is not None:
                sampler = qmc.Sobol(d=1, scramble=True, seed=self.random_state)
            return sampler.random(n).flatten()
        except ImportError:
            return self._van_der_corput_sequence(n, base=2)

    def _van_der_corput_sequence(self, n: int, base: int = 2) -> np.ndarray:
        """Simple van der Corput sequence as fallback."""
        sequence = np.zeros(n)
        for i in range(n):
            n_i = i
            fraction = 0
...
class AliasMethodSampler(BaseInverseTransformSampler):
    """
    Alias method for fast discrete distribution sampling.
    ...
    def __init__(
        ...
        values : np.ndarray, optional
            Corresponding values. If None, uses indices.
        random_state : int, optional
            Random seed for reproducibility.
        """
        super().__init__(random_state)
        self.probabilities = np.array(probabilities) / np.sum(probabilities)
        self.values = values if values is not None else np.arange(len(probabilities))
    ...
    def _get_inverse_cdf(self) -> Callable:
        ...
        raise NotImplementedError("Alias method doesn't use inverse CDF")

    def _build_alias_tables(self):
        """Build the alias and probability tables."""
        scaled_probs = self.probabilities * self.n
        ...
        while small and large:
            ...
            scaled_probs[large_idx] = (
                scaled_probs[large_idx] + scaled_probs[small_idx] - 1.0
            )
    ...
    def sample(self, n: int, random_state: Optional[int] = None) -> np.ndarray:
        """Override sample method for alias method."""
        seed = random_state if random_state is not None else self.random_state
        if seed is not None:
            np.random.seed(seed)

        indices = np.random.randint(0, self.n, n)
        u = np.random.uniform(0, 1, n)

        mask = u < self.prob[indices]
        result_indices = np.where(mask, indices, self.alias[indices])

        return self.values[result_indices]
