...
class MetropolisHastingsSampler:
    ...
    def sample(
        ...
        if len(self.proposal_scale) == 1:
            self._current_scales = np.full((n_chains, self._n_dim), self.proposal_scale[0])
        else:
            if len(self.proposal_scale) != self._n_dim:
                raise ValueError(f"proposal_scale length {len(self.proposal_scale)} "
        ...
        if self.var_names is None:
            ...
            raise ValueError(f"var_names length {len(self.var_names)} "
                           f"doesn't match dimension {self._n_dim}")
                           
        # Storage
        total_iterations = burn_in + n_samples * thin
        posterior_samples = {
            name: np.zeros((n_chains, n_samples)) 
    ...
    def _setup_initial_states(
        ...
        if initial_states is None:
            ...
            if initial_states.shape[0] == 1 and n_chains > 1:
                ...
                perturbations = rng.standard_normal((n_chains, initial_states.shape[1])) * 0.1
    ...
    def _run_chain(
        ...
        for iteration in range(total_iterations):
            ...
            if (self.adaptive_scaling and 
                iteration < burn_in and 
                len(recent_accepts) >= self.adaptation_window):
            ...
            if iteration >= burn_in and (iteration - burn_in) % thin == 0:
                for dim_idx, var_name in enumerate(self.var_names):
                    if self._n_dim == 1:
                        posterior_samples[var_name][chain_idx, sample_idx] = current_state[0]
                    else:
                        posterior_samples[var_name][chain_idx, sample_idx] = current_state[dim_idx]
                        
                sample_stats["log_likelihood"][chain_idx, sample_idx] = current_log_prob
                sample_stats["accepted"][chain_idx, sample_idx] = accept
                sample_stats["proposal_scale"][chain_idx, sample_idx] = self._current_scales[chain_idx]
                
                sample_idx += 1
                
                if pbar is not None:
                    current_accept_rate = np.mean(sample_stats["accepted"][chain_idx, :sample_idx])
                    pbar.set_postfix(
                        samples=sample_idx,
