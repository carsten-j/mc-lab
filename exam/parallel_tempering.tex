...
\begin{frame}{The Temperature Mechanism}
	...
	\begin{columns}
		...
		\textbf{Properties:}
		\begin{itemize}
			\item $T = 1$: Original target distribution
			\item $T > 1$: "Heated" - flatter distribution
			\item $T \to \infty$: Approaches uniform
			\item $T < 1$: "Cooled" - more peaked
		\end{itemize}
...
\begin{frame}{Parallel Tempering Algorithm Prerquisites}
	\begin{itemize}
		...
		\item \textbf{Proposal Distribution}: $q(x' | x)$ - could potentially depend on temperature
...
\begin{frame}{Parallel Tempering Algorithm}
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			% \REQUIRE target $\pi$, proposal $q$, $N$, number of sample for each chain $T$, $s$, $\{\gamma_n\}_{n=1}^N$ with $\gamma_N = 1$
			% \ENSURE Samples from $\pi$
			% \STATE Initialize: $x_n^{(0)}$ for $n = 1, \ldots, N$
			\FOR{$t = 1$ \TO $T$}
			\FORALL{$n \in \{1, \ldots, N\}$ \textbf{ in parallel}}
			\STATE Sample $x_n^{(t)}$ using a standard MCMC step targeting $\pi^{\gamma_n}$
			% \STATE $x_n' \sim q(\cdot | x_n^{(t-1)})$
			% \STATE $\alpha_n = \min\left\{1, \frac{\pi(x_n')^{\gamma_n} q(x_n^{(t-1)} | x_n')}{\pi(x_n^{(t-1)})^{\gamma_n} q(x_n' | x_n^{(t-1)})}\right\}$
			% \STATE $x_n^{(t)} = x_n' \text{ w.p. } \alpha_n \text{, and } x_n^{(t-1)} \text{ w.p. } 1-\alpha_n$
			\ENDFOR
			\IF{$t \bmod s = 0$}
...
\begin{frame}{Swap Acceptance Ratio Derivation}
	...
	\begin{align*}
		...
		% &= \min\left\{1, \frac{[\pi(x_{k_2})]^{\gamma_{k_1}} \cdot [\pi(x_{k_1})]^{\gamma_{k_2}}}{[\pi(x_{k_1})]^{\gamma_{k_1}} \cdot [\pi(x_{k_2})]^{\gamma_{k_2}}}\right\} \\
...
% Section 4: Temperature Selection
